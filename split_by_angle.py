...
def create_list(paths, list_path):
	assert len(paths) > 0
...
def split_by_angle(data_dir, img_list, angle = 10):
	...
	paths, locations, licenses = load(paths)			

	splits = [[],[],[]]

	for i in range(len(locations)):
		angi = angle_from_location(locations[i])
		if angi < -angle:
	...
	create_list(splits[0], out_list)
	out_list = '{}_m{}.txt'.format(list_prefix, angle)	
	create_list(splits[1], out_list)
	out_list = '{}_r{}.txt'.format(list_prefix, angle)	
	create_list(splits[2], out_list)
